...
class DcmntComplianceSectionState extends State<DcmntComplianceSection>
  ...
  @override
  void dispose() {
    _ndasController.dispose(); _employmentAgreementsController.dispose();
    _offerLettersController.dispose();
    _verDayController.dispose(); _verMonthController.dispose(); _verYearController.dispose();
    _ndasFocus.dispose(); _employmentAgreementsFocus.dispose(); _offerLettersFocus.dispose();
    super.dispose();
  }
  ...
  bool validate() {
    bool valid = true;
    setState(() {
      _ndasError = _ndasController.text.trim().isEmpty ? "Please enter NDAs" : null;
      _employmentAgreementsError = _employmentAgreementsController.text.trim().isEmpty ? "Please enter employment agreements" : null;
      _offerLettersError = _offerLettersController.text.trim().isEmpty ? "Please enter offer letters" : null;
      _verificationStatusError = _verificationStatus == null ? "Please select verification status" : null;
      _verificationDateError = _verificationDate == null ? "Please select verification date" : null;

      if (_ndasError != null || _employmentAgreementsError != null ||
          _offerLettersError != null || _verificationStatusError != null ||
  ...
  Widget _buildFileUploadSection({required String label, required List<_UploadedFile> files, required VoidCallback onAdd, required void Function(int) onDelete}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _fieldLabel(label),
      ...files.asMap().entries.map((entry) => _buildFileTile(file: entry.value, onDelete: () => onDelete(entry.key))),
      GestureDetector(onTap: onAdd,
          child: Container(width: double.infinity, padding: EdgeInsets.symmetric(vertical: 10.h, horizontal: 12.w),
              ...
              child: Row(children: [
                ...
                SizedBox(width: 8.w),
                Text('Tap to upload file', style: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFFB8BEC5))),
              ]))),
      SizedBox(height: 10.h),
    ]);
  }

  Widget _buildFileTile({required _UploadedFile file, required VoidCallback onDelete}) {
    return Container(
      ...
      child: Row(children: [
        Container(width: 36.w, height: 36.w,
            decoration: BoxDecoration(color: const Color(0xFFE8ECF4), borderRadius: BorderRadius.circular(6.r)),
            child: Icon(Icons.insert_drive_file_outlined, size: 20.sp, color: const Color(0xFF0A0258))),
        SizedBox(width: 10.w),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(file.name, style: GoogleFonts.inter(fontSize: 12.sp, fontWeight: FontWeight.w500, color: const Color(0xFF303030))),
          Text(file.size, style: GoogleFonts.inter(fontSize: 10.sp, color: const Color(0xFF9CA3AF))),
        ])),
        GestureDetector(onTap: onDelete, child: Padding(padding: EdgeInsets.all(6.r),
    ...
    showModalBottomSheet(
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setModal) => SafeArea(
          child: Container(
            height: 420, padding: const EdgeInsets.fromLTRB(15, 15, 15, 25),
            ...
            child: Column(children: [
              Expanded(child: SfDateRangePicker(
                ...
                view: DateRangePickerView.month, allowViewNavigation: true, showNavigationArrow: true,
                backgroundColor: Colors.white, selectionColor: const Color(0xFF0A0258), todayHighlightColor: const Color(0xFF0A0258),
                ...
                onSelectionChanged: (args) { if (args.value is DateTime) setModal(() => tempDate = args.value); },
              )),
              SizedBox(height: 10.h),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                TextButton(onPressed: () => Navigator.pop(context), child: Text("Cancel", style: GoogleFonts.inter(color: Colors.red, fontSize: 14.sp))),
                TextButton(
                    onPressed: () {
                      setState(() {
  ...
  Widget _buildDateRow() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(child: GestureDetector(onTap: () => _showVerificationDatePicker(context),
            child: Container(
                decoration: BoxDecoration(color: const Color(0xFFF9FAFC),
                    borderRadius: BorderRadius.only(topLeft: Radius.circular(8.r), bottomLeft: Radius.circular(8.r)),
                    border: Border.all(color: _verificationDateError != null ? Colors.red : borderColor)),
                child: IgnorePointer(child: TextField(controller: _verDayController, readOnly: true,
                    style: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFF303030)),
                    decoration: InputDecoration(isDense: true, border: InputBorder.none, hintText: "DD",
        ...
        Expanded(child: GestureDetector(onTap: () => _showVerificationDatePicker(context),
            child: Container(
                decoration: BoxDecoration(color: const Color(0xFFF9FAFC),
                    border: Border(
                        top: BorderSide(color: _verificationDateError != null ? Colors.red : borderColor),
                        bottom: BorderSide(color: _verificationDateError != null ? Colors.red : borderColor),
                        right: BorderSide(color: _verificationDateError != null ? Colors.red : borderColor))),
                child: IgnorePointer(child: TextField(controller: _verMonthController, readOnly: true,
                    style: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFF303030)),
                    decoration: InputDecoration(isDense: true, border: InputBorder.none, hintText: "MM",
                        hintStyle: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFFB8BEC5)),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 8.h))))))),
        Expanded(child: GestureDetector(onTap: () => _showVerificationDatePicker(context),
            child: Container(
                decoration: BoxDecoration(color: const Color(0xFFF9FAFC),
                    ...
                    border: Border(
                        ...
                        bottom: BorderSide(color: _verificationDateError != null ? Colors.red : borderColor),
                        right: BorderSide(color: _verificationDateError != null ? Colors.red : borderColor))),
                child: IgnorePointer(child: TextField(controller: _verYearController, readOnly: true,
                    style: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFF303030)),
                    decoration: InputDecoration(isDense: true, border: InputBorder.none, hintText: "YYYY",
                        hintStyle: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFFB8BEC5)),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 8.h))))))),
      ]),
      if (_verificationDateError != null)
        Padding(padding: EdgeInsets.only(top: 4.h, left: 4.w),
            child: Text(_verificationDateError!, style: GoogleFonts.inter(color: Colors.red, fontSize: 10.sp))),
    ]);
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _buildTextField(controller: _ndasController, isEditing: _isNdasEditing, focusNode: _ndasFocus,
            onEdit: () { if (!_isNdasEditing) setState(() => _isNdasEditing = true); },
            errorText: _ndasError,
            onClearError: () => setState(() => _ndasError = _ndasController.text.trim().isEmpty ? "Please enter NDAs" : null)),
        SizedBox(height: 10.h),
        _fieldLabel('Employment Agreements', required: true),
        _buildTextField(controller: _employmentAgreementsController, isEditing: _isEmploymentAgreementsEditing, focusNode: _employmentAgreementsFocus,
            onEdit: () { if (!_isEmploymentAgreementsEditing) setState(() => _isEmploymentAgreementsEditing = true); },
            errorText: _employmentAgreementsError,
            onClearError: () => setState(() => _employmentAgreementsError = _employmentAgreementsController.text.trim().isEmpty ? "Please enter employment agreements" : null)),
        SizedBox(height: 10.h),
        _fieldLabel('Offer Letters', required: true),
        _buildTextField(controller: _offerLettersController, isEditing: _isOfferLettersEditing, focusNode: _offerLettersFocus,
            onEdit: () { if (!_isOfferLettersEditing) setState(() => _isOfferLettersEditing = true); },
            errorText: _offerLettersError,
            onClearError: () => setState(() => _offerLettersError = _offerLettersController.text.trim().isEmpty ? "Please enter offer letters" : null)),
        SizedBox(height: 16.h),
        _sectionHeading('Identification'),
        _buildFileUploadSection(label: 'Scans of Passports', files: _passportFiles,
            onAdd: () => _addMockFile(_passportFiles), onDelete: (i) => setState(() => _passportFiles.removeAt(i))),
        _buildFileUploadSection(label: 'Visas', files: _visaFiles,
            onAdd: () => _addMockFile(_visaFiles), onDelete: (i) => setState(() => _visaFiles.removeAt(i))),
        _buildFileUploadSection(label: "Driver's Licenses", files: _driversLicFiles,
            onAdd: () => _addMockFile(_driversLicFiles), onDelete: (i) => setState(() => _driversLicFiles.removeAt(i))),
        SizedBox(height: 6.h),
        _sectionHeading('Background Checks'),
        _fieldLabel('Verification Status', required: true),
        DropdownButtonFormField<String>(
          value: _verificationStatus,
          hint: Text('Select status', style: GoogleFonts.inter(fontSize: 12.sp, color: const Color(0xFF6C7278))),
          ...
          decoration: InputDecoration(isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
              filled: true, fillColor: const Color(0xFFF9FAFC), errorStyle: const TextStyle(fontSize: 0, height: 0),
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(8.r), borderSide: const BorderSide(color: Color(0xFFD9DEE5))),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8.r),
                  borderSide: BorderSide(color: _verificationStatusError != null ? Colors.red : const Color(0xFFD9DEE5))),
          ...
          onChanged: (v) => setState(() { _verificationStatus = v; _verificationStatusError = null; }),
        ),
        if (_verificationStatusError != null)
          Padding(padding: EdgeInsets.only(top: 4.h, left: 4.w),
              child: Text(_verificationStatusError!, style: GoogleFonts.inter(color: Colors.red, fontSize: 10.sp))),
        SizedBox(height: 10.h),
